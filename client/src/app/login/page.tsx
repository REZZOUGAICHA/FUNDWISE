...
const handleSubmit = async (e: { preventDefault: () => void; }) => {
  ...
  try {
    // Call the auth service through API Gateway
    ...
    if (response.data.user && response.data.user.role) {
      ...
      // If we can't find the role in the expected location, try to decode it from the token
      try {
        const tokenParts = response.data.token.split('.');
        const payload = JSON.parse(atob(tokenParts[1]));
        if (payload.role) {
          localStorage.setItem('userRole', payload.role);
          console.log('User role extracted from token:', payload.role);
        } else {
          console.warn('No user role found in token payload');
        }
      } catch (error) {
        console.error('Error extracting role from token:', error);
      }
    }
    console.log('User role:', localStorage.getItem('userRole')|| "public");
