...
export class VerificationService {
  ...
  async approveOrganization(orgId: string) {
    return this.prisma.$executeRawUnsafe(
      `UPDATE auth_service.organizations SET verification_status = 'approved' WHERE id = $1`,
      orgId
    );
  }
  ...
  async approveCampaign(campaignId: string) {
    return this.prisma.$executeRawUnsafe(
      `UPDATE campaign_service.campaigns SET status = 'active' WHERE id = $1`,
      campaignId
    );
  }
  ...
  async approveProof(proofId: string) {
    return this.prisma.$executeRawUnsafe(
      `UPDATE campaign_service.proofs SET status = 'approved' WHERE id = $1`,
      proofId
    );
  }
